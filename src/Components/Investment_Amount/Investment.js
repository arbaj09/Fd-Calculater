...
const Investment = () => {
  ...
  const calculatelumpsumHandler=()=>{
    ...
    if(Value.TwoOption==='AsLumpsum'){
      ...
      const n = periodsPerYear * duration;
      const r = parseFloat(Value.Interest) / 100;
      const M = principal * Math.pow(1 + r, n);
     ...
     <div className="Amount">
      ...
      <div className="Period">
       ...
       { ShowTarget ?(<div className="Container-invested">
          ...
          <button onClick={calculateCompoundInterest} className="Calculate">Calculate</button>
        ...
        <button onClick={calculatelumpsumHandler} id="Lumpsum"> click here {Value.TwoOption}</button></div>)
        }
      </div>
